...
import { resetProductDetail, setCurrentCustomerCartDetail, setProductDetail } from '../Store/cartSlice';
import LoaderTwo from '../Components/Loader/SecondLoader/LoaderTwo';

function ProductDetailPage() {
      const params = useParams()
      const dispatch = useDispatch()
      const currentCustomer = useSelector((state) => state?.customer?.customer)
      const prevCartData = useSelector((state) => state?.addTocart?.itemsData)
            ...
            if (currentCustomer) {
                  try {
                        ...
                        if (serializedState !== null) {
                              ...
                              const currentCartItems = cartData.itemsData
                              const newAddedProduct = {}
                              for (let key in currentCartItems) {
                                    if (prevCartData.hasOwnProperty(key)) {
                                          if ((currentCartItems[key].quantity - prevCartData[key].quantity) > 0) {
                                                newAddedProduct[key] = {
                                                      quantity: currentCartItems[key].quantity - prevCartData[key].quantity,
                                                      price: currentCartItems[key].price
                                                }
                                          }
                                    }
                                    else {
                              ...
                              const backendAPIResponse = await fetch(backendRoutesAPI.loggedInCustomerCartDetail.url, {
                                    ...
                                    body: JSON.stringify({ cartData: newAddedProduct })
                              })
                              ...
                              if (finalRes.success) {
                                    ...
                                    getCustomerCartData(finalRes.message)
                              }
                        }
                  } catch (err) {
      ...
      const getCustomerCartData = async (message) => {
